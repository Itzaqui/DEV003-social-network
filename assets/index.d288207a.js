...
const deletePost = (id2) => Gl(Aa(db, "post", id2));
const addLike = (id2, userId) => {
  Kl(Aa(db, "post", id2), { likes: of(userId) });
};
const removeLike = (id2, userId) => {
  Kl(Aa(db, "post", id2), { likes: uf(userId) });
};
const createPost = (data) => Ql(Ta(db, "post"), data);
const getPost = (id2) => Ol(Aa(db, "post", id2));
const updatePost = (id2, newFields) => Kl(Aa(db, "post", id2), { ...newFields });
const orderedPost = (callback) => {
  const q2 = sl(Ta(db, "post"), hl("time", "desc"));
  jl(q2, callback);
};
const myFunction = () => {
  console.log("Hola mundo!");
};
const addRouteEvents = () => {
...
const cakebook = () => {
  ...
  <div class="containerTimeline">
    <form id="form-post" class="myPosts">
       <h2 class="titulo">\xBFQu\xE9 vamos a compartir hoy?</h2>
    ...
    <template id="posts">
      <div class="post">
        ...
        <div>
          ...
          <p id="description">Aqu\xED va el post</p>
        </div>
      ...
      <div class="modal-deletePost">
        <div class="modal-content">
          <div class="close-title">
              <p class="modal-title">\xBFEliminar publicaci\xF3n?</p> 
              <div class="btn-modal">
                <button class="closed-modal">X</button>
              </div> 
          </div>
          <p class="text-deletePost">\xBFSeguro que quieres eliminar esta publicaci\xF3n?</p>
          <div class="btn-modal">
  ...
  const loadPosts = (data) => {
    ...
    if (data) {
      data.forEach((doc) => {
        ...
        spanSumLikes.textContent = ((_a2 = dataPost.likes) == null ? void 0 : _a2.length) || "";
        h2userName.textContent = dataPost.userName;
        pDescription.textContent = dataPost.description;
        pTime.textContent = ((_b = dataPost.time) == null ? void 0 : _b.toDate().toLocaleString()) || "";
        const textPublication = document.getElementById("texto");
        ...
        if ((_c2 = dataPost.likes) == null ? void 0 : _c2.includes(auth.currentUser.uid)) {
      ...
      containerListPosts.textContent = "No hay publicaci\xF3n";
    }
  };
  function eventDelete() {
    ...
    btnsDelete.forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        btnDeletePost.addEventListener("click", (e2) => {
          const id2 = btnDeletePost.getAttribute("data-id");
          deletePost(id2);
        });
      });
    });
...
function loginWithGoogle(e) {
  ...
  signInWithPopup(auth, provider).then(autenticGoogle).catch(handleError$1);
}
const register = () => {
  const viewRegister = `
    ...
    <h1>cakeBook</h1>
    <div class="saludo">
      <span>Reg\xEDstrate para ver y compartir</span> 
    ...
    <form class="form-btn">
      <input class= "input-users" type="email" name="email" id="userEmail" placeholder="Correo electr\xF3nico">
      ...
      <input class= "input-users" type="password" name="password" id="password"  placeholder="Contrase\xF1a">
      <div class="span-politica">
        <span>Al registrarte, aceptas nuestras Condiciones, la </span> 
        <span>Pol\xEDtica de privacidad y la Pol\xEDtica de cookies.</span>
      </div>
...
function handleError(error) {
  ...
  const errors = {
    ...
    "auth/invalid-email": "Correo inv\xE1lido.",
    "auth/email-already-in-use": "Correo ya registrado.",
    "auth/weak-password": "Por favor, introduce una contrase\xF1a que contenga m\xE1s de 6 car\xE1cteres.",
    "auth/internal-error": "Por favor, introduce una contrase\xF1a."
  };
...
function registerWithEmailAndPassword(e) {
  ...
  register$1(data.email, data.password).then((userCredential) => registeredUser(userCredential, data)).catch(handleError);
}
const init$3 = () => {
  const form = document.querySelector(".form-btn");
  ...
  const viewLogin = `
    ...
    <div class= "container1">
      ...
      <form class= "form-login" id="form-login">
        <input class= "input-users" type="email" name="userEmail" id="userEmail" placeholder="Correo electr\xF3nico" required>
        <input class= "input-users" type="password" name="password" id="password"  placeholder="Contrase\xF1a" required>
        <button class="log-button" id="login">INICIAR SESI\xD3N</button>
...
function loginWithEmailAndPassword(e) {
  ...
  login$1(email, password).then(() => {
    history.pushState(null, null, "/cakebook");
  }).catch((error) => {
    const errorCode = error.code;
    const errors = {
      "auth/wrong-password": "Contrase\xF1a inv\xE1lida",
...
const profile = () => {
  ...
  <h4 id="profInfo">Edad, descripci\xF3n de perfil</h4>
  <button type="button" class="editProfile" id="editProfile">Editar perfil</button>
  </div>
  <div class="containerProfile">
    <form id="form-post" class="myPosts">
       <h3>\xBFQu\xE9 vamos a compartir hoy?</h3>
        <div class="header-post"> 
          <div class="img-perfil">
            <img src="./img/userPic.png" alt="" class="imgUser">
          </div>
...
const routes = {
  login: { view: login, init: init$2 },
  cakebook: { view: cakebook, init: init$4 },
  register: { view: register, init: init$3 },
  profile: { view: profile, init: init$1 }
};
